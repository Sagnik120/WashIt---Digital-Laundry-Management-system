...
import { ApiUrlCommon, BaseUrl, LOGIN_TOKEN, REFRESH_TOKEN, versionControl } from './AuthApi';
...
export async function handleRequest(request) {
	let login_token = cookieUtils.getCookie(LOGIN_TOKEN);
	...
	if (login_token) {
		...
		if (Math.floor(new Date().getTime() / 1000) >= decodedJwt?.exp - 5 * 60) {
			...
			await fetch(BaseUrl + ApiUrlCommon + versionControl + 'ConsultantPortal/RefreshToken', {
				method: 'POST',
				headers: {
